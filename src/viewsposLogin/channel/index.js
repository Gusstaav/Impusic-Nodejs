...
voltar.innerHTML=`<a href='/homeuserid/${iduser}' class="voltar">Voltar</a>`
...
function Channel(elements, datas){
    ...
    //link para editar o canal
    var divEditChannel =  window.document.createElement('div')
    divEditChannel.className='editChannel'
   
    var linkEditChannel =  window.document.createElement('a')
    linkEditChannel.className='editLink'
    linkEditChannel.href='/editChannel/'+iduser+'/'+dataChannel.id
    linkEditChannel.innerText='Editar canal'

    divEditChannel.appendChild(linkEditChannel)
   ...
   if(!dataChannel.urlimageperfil){
    ...
    NewContainer.appendChild(divEditChannel)
